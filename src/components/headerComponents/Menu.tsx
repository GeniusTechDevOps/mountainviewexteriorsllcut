...
export default function Menu({
  ...
  return (
    <div className={`flex flex-col md:flex-row items-center md:items-start md:gap-3 gap-[24px] ${styleList}`}>
      {routes.map((item, index) => {
        if (item.statusRoutes === undefined || item.statusRoutes === true) {
          return (
            <div
              key={index}
              className={`relative content_subItems pb-2 z-50 md:border-none border-r-0 md:border-b-2 border-white md:w-auto w-[200px] ${colorText}`}
            >
              {item.child ? (
                <>
                  <span
                ...
                <a href={item.path} onClick={clickEvent} className="">
                  <span
                    className="font-bold border-r-0 md:border-r pr-2 border-black text-[17px] md:cursor-pointer md:transition-all md:duration-500 md:ease-in-out hover:text-secondary"
                  >
                    {item.name}
                  </span>
                </a>
